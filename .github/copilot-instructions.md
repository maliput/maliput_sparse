...
│   ├── loader/                     # RoadGeometry / RoadNetwork / Rules loaders
...
Each `parser::Lane` may optionally include a `std::vector<SpeedLimit> speed_limits` field describing speed limit zones along the lane's s-coordinate. The `SpeedLimit` struct contains `s_start`, `s_end`, `min`, `max` (in m/s), `description`, and `severity`. When provided, the `RoadRulebookLoader` converts these into maliput `RangeValueRule` entries.
...
#### `loader::RoadGeometryLoader` / `RoadNetworkLoader` / `RoadRulebookLoader` (loader/)
...
- `RoadRulebookLoader`: Takes a `const Parser*`, populates a `RuleRegistry` + `RoadRulebook` with rules extracted from parsed lane data (e.g., speed limits). Extensible — new rule types are added as private methods called from `operator()`
...
| **Speed limit rules** | Parsed from `parser::Lane::speed_limits` via `RoadRulebookLoader`; registered as `RangeValueRule` with `SpeedLimitRuleTypeId()` |
...
3. **Lane data format**: Each `parser::Lane` requires:
   ...
   - `speed_limits`: Optional vector of `SpeedLimit` structs (s_start, s_end, min, max, description, severity)
...
- `test/loader/road_network_loader_test.cc`, `test/loader/road_rulebook_loader_test.cc`
